...
function subgenerator_test_plugin_autoload_classes( $class_name ) {
	...
	if ( 0 !== strpos( $class_name, 'STP_' ) ) {
		return;
	}

	// Set up our filename.
	$filename = strtolower( str_replace( '_', '-', substr( $class_name, strlen( 'STP_' ) ) ) );

	// Include our file.
	Subgenerator_Test_Plugin::include_file( 'includes/class-' . $filename );
}
spl_autoload_register( 'subgenerator_test_plugin_autoload_classes' );
...
final class Subgenerator_Test_Plugin {
	...
	public function init() {
		...
		load_plugin_textdomain( 'subgenerator-test-plugin', false, dirname( $this->basename ) . '/languages/' );
	...
	public function requirements_not_met_notice() {
		...
		$default_message = sprintf( __( 'Subgenerator Test Plugin is missing requirements and has been <a href="%s">deactivated</a>. Please make sure all requirements are available.', 'subgenerator-test-plugin' ), admin_url( 'plugins.php' ) );
...
function subgenerator_test_plugin() {
	return Subgenerator_Test_Plugin::get_instance();
}

// Kick it off.
add_action( 'plugins_loaded', array( subgenerator_test_plugin(), 'hooks' ) );

// Activation and deactivation.
register_activation_hook( __FILE__, array( subgenerator_test_plugin(), '_activate' ) );
register_deactivation_hook( __FILE__, array( subgenerator_test_plugin(), '_deactivate' ) );
